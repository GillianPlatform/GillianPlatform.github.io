---
title: Gillian, Part I - Implementation differences
---

# Gillian, Part I - Differences between Paper and Implementation
...
type 'label t =
  ...
  | Call          of string * Expr.t * Expr.t list * 'label option *
                     logic_bindings_t option                          (** Procedure call                    *)
...
- `goto [e] lab1 lab2` that jumps either to `lab1` or to `lab2` depending on the boolean `e` evaluates to
  Note that using `ifgoto` or these two kinds of gotos is equivalent. Moreover, in the implementation, the type of labels is polymorphic. `string Cmd.t` corresponds to "labelled commands", meaning one can annotate commands with string labels. `int Cmd.t` corresponds to "indexed commands", meaning `goto j` jumps the the `j`-th command of the current procedure. We write GIL programs with labelled commands for readability, and translate to indexed commands for efficiency.
...
As explained earlier, there is no defined set `A` of actions, actions are denoted by their name, a string. Also, the procedures and programs contain much more information than what is in the paper.
...
> **Definition** *(Symbolic Memory Model)*: A symbolic memory model, $\hat M \in \mathbb{M}$, is a triple $\langle |\hat M|, A, \hat{\underline{\mathsf{ea}}}\rangle$, consisting of a set of symbolic memories, $|\hat M| \ni \hat \mu$, a set of actions $A \ni \alpha$, and the action execution function $\underline{\mathsf{ea}} : A \rightarrow |M| \rightarrow \mathcal{V} \rightarrow \wp(|M| \times \mathcal{V})$, pretty-printed $\mu.\alpha(v) \rightsquigarrow (\mu', v)$, pretty-printed $\hat \mu.\alpha(\hat e) \rightarrow (\mu', \hat e', \pi ')$.

In the implementation, the these memory models is a bit more complex. The complete interface can be found in the files `GillianCore/engine/SymbolicSemantics/SMemory.ml` and `GillianCore/engine/ConcreteSemantics/CMemory.ml`.
...
In the implementation, the interface of state models, available in `GillianCore/engine/GeneralSemantics/State.ml` is a bit different and more complex.
